...
canvas.bind("<Double-Button-1>", lambda e: set_mode("text") or insert_text(e))
...
def insert_text(event):
    text = simpledialog.askstring("Nhập văn bản", "Nhập nội dung:")
    if text:
        text_id = canvas.create_text(event.x, event.y, text=text,
                                   font=("Arial", 16),
                                   fill=current_color)
        # Lưu thông tin đầy đủ về văn bản
        drawn_objects.append({
            'type': 'text',
            'id': text_id,
